...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
                 datatype="lmdb", build=False):
        self.mode = mode # train
        self.ng = num_gloss # -1
        self.prefix = prefix # phoenix-2014-multisigner
        self.dict = gloss_dict 
        self.data_type = datatype # video
        self.feat_prefix = f"{prefix}/features/fullFrame-256x256px/{mode}" # train
        if build:
            self.transform_mode = "build"
        elif transform_mode:
            self.transform_mode = "train"
        else:
            self.transform_mode = "test"
    ...
    def __getitem__(self, idx):
        if self.data_type == "video":
            ...
            input_data, label, index = self.normalize(input_data, label)
            # input_data, label = self.normalize(input_data, label, fi['fileid'])
            return input_data, torch.LongTensor(label), self.inputs_list[idx]['original_info'], index
            ...
            return input_data, label, self.inputs_list[idx]['original_info'], index
    ...
    def normalize(self, video, label, file_id=None):
        video, label, index = self.data_aug(video, label, file_id)
    ...
    def collate_fn(batch):
        ...
        video, label, info, index = list(zip(*batch))
        if len(video[0].shape) > 3:
            ...
            # if index[0] != None:
                # for idx in range(len(index)):
                    # left_index = [index[idx][0]] * left_pad
                    # right_index = [index[idx][-1]] * (max_len - len(index[idx])- left_pad)
                    # padded_index.append(left_index + index[idx] + right_index)
        else:
            max_len = len(video[0])
            video_length = torch.LongTensor([len(vid) for vid in video])
            padded_video = [torch.cat(
        ...
        if max(label_length) == 0:
            return padded_video, video_length, [], [], info, index
        else:
            padded_label = []
            for lab in label:
                padded_label.extend(lab)
            padded_label = torch.LongTensor(padded_label)
            return padded_video, video_length, padded_label, label_length, info, tuple(padded_index)
