...
def build_dataloader(dataset, mode, train_flag, arg):
    return torch.utils.data.DataLoader(
        dataset,
        batch_size=arg['batch_size'] if mode.split("_")[0] == "train" else arg['test_batch_size'],
        shuffle=train_flag,
        drop_last=True if mode in ["train", "dev", "test"] else False,
        num_workers=arg['num_worker'],
        collate_fn=dataset.collate_fn
    )

def load_cfg(yaml_dir):
