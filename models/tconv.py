    ...
    def update_lgt(self, lgt):
        ...
        for ks in self.kernel_size:
            if ks[0] == 'P':
                ...
                feat_len -= int(ks[1]) - 1
                feat_len += 4 # padding 2
        return feat_len

    def forward(self, frame_feat, lgt, key):
        visual_feat = self.temporal_conv(frame_feat) # batch, embed, temp
        lgt = self.update_lgt(lgt)

        if key:
            return {
                "visual_feat": visual_feat.transpose(1, 2),
                "conv_logits": self.fc(visual_feat.transpose(1, 2)),
                "feat_len": lgt.cpu(),
            }
        else:        
            return {
